...
static DWORD WINAPI th(LPVOID lpParameter)
	...
	MessageBox(NULL, (LPCWSTR)L"Thread entered.", (LPCWSTR)L"It's working...", MB_OK | MB_ICONEXCLAMATION);
...
HANDLE SetHardwareBreakpoint(HANDLE hThread,HWBRK_TYPE Type,HWBRK_SIZE Size,void* s)
	...
	sprintf(threadId, "%u", GetLastError());

	MessageBoxA(NULL, (LPCSTR)threadId, (LPCSTR)"It's working...", MB_OK | MB_ICONEXCLAMATION);

	sprintf(threadId, "%lu", GetThreadId(hY));
	...
	MessageBox(NULL, (LPCWSTR)L"Thread returned.", (LPCWSTR)L"It's working...", MB_OK | MB_ICONEXCLAMATION);
